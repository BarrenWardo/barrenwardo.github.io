...
**Second audit pass (5 agents, post-ship).** Found and fixed one ship-blocker the first pass missed — the echo constructor wrote `this.probe.done` before `this.probe` existed, so `init("echo")` always threw into the `unsupported` catch — plus hardening: input validation on the `inject`/`stir`/`tint` choke points, full observer/listener/GL cleanup on `destroy()`/`fail()`, init recovery after dead instances, WebGL1 unfilterable-half-float fallback, realloc skip on unchanged grids, `probeSkip` after tier steps, deferred-all-autonomous batching, spec-curve bloom, snapshot crossfade total, driver parking when idle, mid-session reduced-motion freeze, no-GSAP fluid startup, `IntersectionObserver` visibility fallback without ScrollTrigger, and explicit canvas hit-test ownership in CSS. Deliberately unchanged: solver math (gradient 0.5 factor, pressure-decay units) — needs rendered-output verification before retuning; `coVisible`/`lastFrameAt` dead fields — harmless.
